...
import { TopBar, SourceTag, FieldGroup } from "./Shared";
import { COUNTRIES, NOTIFICATIONS } from "../data";
import { downloadPassport } from "../utils/passportPdf";

const SCREENS = ["profile", "passport", "opportunities", "training"] as const;
type Screen = typeof SCREENS[number];

interface InferredSkill { label: string; code: string; kind: "durable" | "vulnerable"; evidence: string; }
interface SkillRule extends InferredSkill { key: string[]; }

function inferSkills(text: string): InferredSkill[] {
  const t = text.toLowerCase();
  const rules: SkillRule[] = [
    { key: ["fix","repair","mobile","phone","circuit","board","screen","battery"], label: "Mobile electronics repair",            code: "ESCO ▸ S6.2.1 / O*NET 49-2094", kind: "durable",    evidence: "Hands-on, with place and tools." },
    { key: ["customer","client","explain","calm"],                                 label: "Customer service & complaint handling", code: "ESCO ▸ S1.6.1",                 kind: "durable",    evidence: "High-context, de-escalating language with customers." },
    { key: ["notebook","inventory","track","parts","stock"],                       label: "Inventory & stock tracking",            code: "ESCO ▸ S5.6.2",                 kind: "vulnerable", evidence: "Notebook system; structurally automatable." },
    { key: ["whatsapp","post","social","share"],                                   label: "Social media micro-marketing",          code: "ESCO ▸ S5.5.4",                 kind: "vulnerable", evidence: "Posts shop on WhatsApp; growing skill area." },
    { key: ["uncle","stall","shop","market"],                                      label: "Small-business / shop floor ops",        code: "O*NET 41-1011 (related)",        kind: "durable",    evidence: "Multi-role: people, money, decisions on a stall." },
    { key: ["solar","panel","install"],                                            label: "Solar panel installation",              code: "O*NET 47-2231",                 kind: "durable",    evidence: "Hands-on field installation with physical tools." },
    { key: ["nurse","care","patient"],                                             label: "Primary care / community health",       code: "ISCO 3221",                     kind: "durable",    evidence: "Direct patient contact and community outreach." },
    { key: ["weld","cnc","machine"],                                               label: "Manufacturing — machine setup",         code: "O*NET 51-4011",                 kind: "durable",    evidence: "Tactile precision work; hard to fully automate." },
  ];
  return rules.filter(r => r.key.some(k => t.includes(k))).slice(0, 6);
}

export default function PortalYouth({ onHome }: { onHome: () => void }) {
  ...
  const tabs = [
    { id: "profile" as Screen,       n: "01", l: "Tell us about you" },
    { id: "passport" as Screen,      n: "02", l: "Your skills passport" },
    { id: "opportunities" as Screen, n: "03", l: "Your opportunities" },
    { id: "training" as Screen,      n: "04", l: "Your training paths" },
  ];

  return (
    <div style={{ minHeight: "100vh", background: "var(--paper)" }}>
      <TopBar portal="For young people" onHome={onHome}
        country={country} onCountryChange={setCountry}
        notifications={NOTIFICATIONS.youth} />
      ...
      <div style={{ background: "var(--paper-2)", borderBottom: "1px solid var(--line-soft)", padding: "14px 28px" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", gap: 6, flexWrap: "wrap" }}>
          {tabs.map(t => {
            ...
            const passed = SCREENS.indexOf(t.id) < SCREENS.indexOf(screen);
            return (
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 28px 80px" }} className="fadein" key={screen}>
        ...
        {screen === "passport"      && <YouthPassport name={name} story={story} country={country} edu={edu} age={age} onNext={() => setScreen("opportunities")} />}
        {screen === "opportunities" && <YouthOpportunities country={country} onNext={() => setScreen("training")} />}
        {screen === "training"      && <YouthTraining onHome={onHome} />}
      </div>
    </div>
  );
}
...
function YouthProfile({ story, setStory, name, setName, age, setAge, edu, setEdu, country, onNext }: {
  story: string; setStory: (v: string) => void; name: string; setName: (v: string) => void;
  age: number; setAge: (v: number) => void; edu: string; setEdu: (v: string) => void;
  country: string; onNext: () => void;
}) {
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 40, alignItems: "start" }}>
      <div>
        ...
        <h1 className="display" style={{ fontSize: 56, marginBottom: 16, fontWeight: 500 }}>
          Tell us who you are.<br /><span className="serif-italic" style={{ color: "var(--green)" }}>In your own words.</span>
        </h1>
        <p style={{ fontSize: 17, color: "var(--ink-soft)", lineHeight: 1.55, marginBottom: 30, maxWidth: 560 }}>
          Don't worry about job titles or formal language. Tell us what you actually do, the way you'd tell a friend. We'll figure out the labels.
        </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr", gap: 12, marginTop: 18 }}>
          <FieldGroup label="Your name"><input className="field" value={name} onChange={e => setName(e.target.value)} /></FieldGroup>
          <FieldGroup label="Age"><input className="field" type="number" value={age} onChange={e => setAge(+e.target.value)} /></FieldGroup>
            ...
            : detected.map((s, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "var(--paper-2)", borderRadius: 12 }}>
                <div>
                  ...
                  <p className="mono" style={{ margin: 0, fontSize: 11, color: "var(--ink-faint)" }}>{s.code}</p>
                </div>
...
function YouthPassport({ name, story, country, edu, onNext }: { name: string; story: string; country: string; edu: string; age: number; onNext: () => void }) {
  ...
  function handleDownload() {
    downloadPassport({
      ...
      countryName: c.name,
      countryFlag: c.flag,
      countryInformal: c.informal,
      edu,
      skills: skills.map(s => ({ label: s.label, code: s.code, durable: s.kind === "durable", evidence: s.evidence })),
    });
  }
    ...
    <div>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: 28 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button className="btn btn-cream" onClick={handleDownload}>↓ Download as PDF</button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 18, marginBottom: 18 }}>
        <div className="card" style={{ background: "var(--green)", color: "#fff", borderRadius: 22, padding: 28, position: "relative", overflow: "hidden" }}>
          ...
          <p style={{ margin: 0, opacity: 0.85, fontSize: 14 }}>{c.name} · informal sector verified</p>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <p style={{ fontSize: 13, color: "var(--ink-faint)", margin: 0 }}>Want to know how durability is calculated? <SourceTag sourceId="exposure" label="how it's calculated" /></p>
...
function SkillCard({ s }: { s: InferredSkill }) {
  ...
  return (
    <div className="card" style={{ borderColor: isDurable ? "#c8debc" : "#f3c8b6", padding: 0, overflow: "hidden" }}>
      <div style={{ display: "flex", gap: 0 }}>
        <div style={{ width: 80, height: "auto", minHeight: 100, background: "var(--paper-2)", borderRight: "1px solid var(--line-soft)", overflow: "hidden" }}>
    ...
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 16, color: "var(--ink-soft)", maxWidth: 620 }}>
          We didn't show you 47 jobs scored 88/100. We picked four. Real wages, real distance, real honesty about what you'd need to learn.
        </p>
      </div>
...
function OpCard({ o }: { o: { role: string; org: string; wage: string; honest: string; fit: number; gap: string[]; sector: string; training: string } }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="card" style={{ padding: 0, overflow: "hidden" }}>
      <div className="photo-ph" style={{ height: 160, borderRadius: 0, overflow: "hidden" }}>
        <img src={
      ...
      <div style={{ padding: 22 }}>
        ...
        <button onClick={() => setOpen(p => !p)} style={{ marginTop: 14, border: "none", background: "transparent", padding: 0, color: "var(--ink-mute)", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
          {open ? "↑ Hide the technical bit" : "↓ Show the technical bit"}
        </button>
        {open && (
          <div className="fadein" style={{ marginTop: 14, padding: 14, background: "var(--paper-2)", borderRadius: 14, fontSize: 13, lineHeight: 1.6, color: "var(--ink-soft)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ fontWeight: 600 }}>Skill gap</span><span style={{ color: "var(--coral)" }}>{o.gap.join(", ")}</span></div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ fontWeight: 600 }}>Training to close it</span><span>{o.training}</span></div>
            <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 600 }}>Composite score</span><span className="mono" style={{ fontSize: 11 }}>{o.fit.toFixed(2)} (skill_fit·reach·income·sector·resilience)</span></div>
          </div>
        )}
      </div>
    </div>
...
function YouthTraining({ onHome }: { onHome: () => void }) {
  const paths = [
    { name: "Solar Field Technician — TVET Ghana", duration: "6 months · Mon–Fri", cost: "Free (NGO-funded)", provider: "GIZ + ENI", loc: "Tema · in-person", waitlist: false },
    { name: "Digital customer care fundamentals",   duration: "8 weeks · evenings",  cost: "Free",            provider: "ALX Africa", loc: "Online", waitlist: false },
    { name: "ECOWAS Renewables Apprenticeship",     duration: "9 months · paid stipend", cost: "USD 80 fee",  provider: "ECOWAS Centre", loc: "Accra · in-person", waitlist: true },
  ];
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 16, color: "var(--ink-soft)", maxWidth: 620 }}>Real programs, in your country, with real start dates.</p>
      </div>
        ...
        {paths.map((p, i) => (
          <div key={i} className="card" style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 24, alignItems: "center" }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: "var(--ink-mute)", margin: 0 }}>Run by {p.provider}</p>
            </div>
