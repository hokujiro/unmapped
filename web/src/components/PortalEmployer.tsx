...
import { TopBar, SourceTag, FieldGroup, riskColor } from "./Shared";
import { COUNTRIES, TALENT_POOL, NOTIFICATIONS } from "../data";

type EmpScreen = "org" | "post" | "search" | "positions";

interface Props { onHome: () => void; }

export default function PortalEmployer({ onHome }: Props) {
  const [screen, setScreen] = useState<EmpScreen>("org");
  const [country, setCountry] = useState("GHA");
  const [orgKind, setOrgKind] = useState<"employer" | "ngo">("employer");

  const tabs: { id: EmpScreen; n: string; l: string }[] = [
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--paper)" }}>
      <TopBar
        ...
        notifications={NOTIFICATIONS.employer}
      />
      ...
      <div style={{ background: "var(--paper-2)", borderBottom: "1px solid var(--line-soft)", padding: "14px 28px" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between" }}>
            ...
            {tabs.map(t => {
              ...
              return (
                <button key={t.id} onClick={() => setScreen(t.id)} className="tab-btn"
                  style={{ background: active ? "var(--ink)" : "transparent", color: active ? "var(--paper)" : "var(--ink-mute)" }}>
          ...
          <div style={{ display: "flex", gap: 4, padding: 3, background: "#fff", borderRadius: 999, border: "1px solid var(--line)" }}>
            {([["employer", "Employer"], ["ngo", "NGO"]] as const).map(([k, l]) => (
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "40px 28px 80px" }} className="fadein" key={screen + orgKind}>
        ...
        {screen === "post"      && <EmpPost onNext={() => setScreen("search")} />}
...
function EmpOrgProfile({ orgKind, country, onNext }: { orgKind: "employer" | "ngo"; country: string; onNext: () => void }) {
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 40 }}>
      <div>
        ...
        <p style={{ fontSize: 17, color: "var(--ink-soft)", lineHeight: 1.55, marginBottom: 30, maxWidth: 540 }}>
          Just enough so we can pre-tune searches for the kind of talent you usually need.
        </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <FieldGroup label="Country">
            <input className="field" value={c.name} disabled />
          </FieldGroup>
      ...
      <div className="card" style={{ position: "sticky", top: 90, alignSelf: "start" }}>
        ...
        <p style={{ fontSize: 13, color: "var(--ink-mute)", lineHeight: 1.5, marginBottom: 16 }}>
          Based on your sector and mission, we'll bias talent search toward these profiles.
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {inferred.map((x, i) => (
            <div key={i} style={{ padding: "10px 14px", background: "var(--paper-2)", borderRadius: 10, fontSize: 13, color: "var(--ink-soft)", display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ width: 6, height: 6, borderRadius: 999, background: "var(--green)", flexShrink: 0 }}></span>
              {x}
            </div>
          ))}
        </div>
...
function EmpPost({ onNext }: { onNext: () => void }) {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 17, color: "var(--ink-soft)", lineHeight: 1.55, marginBottom: 30, maxWidth: 700 }}>
        We'll handle ESCO codes, ISCO, ISIC sectors and language metadata for you.
      </p>
...
function EmpSearch({ orgKind, country, onNext }: { orgKind: "employer" | "ngo"; country: string; onNext: () => void }) {
  ...
  const filtered = TALENT_POOL.filter(p => {
    ...
    if (filterVuln && orgKind === "ngo" && p.vulnerable.length === 0) return false;
    return true;
  });
    ...
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <h1 className="display" style={{ fontSize: 56, marginBottom: 12, fontWeight: 500 }}>
          {orgKind === "ngo"
            ? <>Find people who could <span className="serif-italic" style={{ color: "var(--green)" }}>benefit.</span></>
            : <>Find your <span className="serif-italic" style={{ color: "var(--green)" }}>next hire.</span></>}
        </h1>
        <p style={{ fontSize: 16, color: "var(--ink-soft)", maxWidth: 620 }}>
          {orgKind === "ngo"
            ...
            : "Profiles are anonymous until you unlock contact. Skills are verified through evidence in their story."}
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 24, alignItems: "start" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 14 }}>
          {filtered.map(p => (
            <ProfileCard key={p.id} p={p} unlocked={!!unlocked[p.id]}
              onUnlock={() => setUnlocked(u => ({ ...u, [p.id]: true }))} orgKind={orgKind} />
          ))}
          {filtered.length === 0 && (
...
function ProfileCard({ p, unlocked, onUnlock, orgKind }: {
  p: typeof TALENT_POOL[0]; unlocked: boolean; onUnlock: () => void; orgKind: "employer" | "ngo";
}) {
  const c = COUNTRIES.find(cc => cc.code === p.country) || COUNTRIES[0];
  return (
    <div className="card" style={{ padding: 0, overflow: "hidden" }}>
      <div style={{ padding: 20, display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "1px dashed var(--line)" }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <div style={{ width: 44, height: 44, borderRadius: 999, background: "var(--green-pale)", color: "var(--green)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 16, flexShrink: 0 }}>
      ...
      <div style={{ padding: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          ...
          {!unlocked
            ? <button className="btn btn-primary" style={{ padding: "8px 14px", fontSize: 12 }} onClick={onUnlock}>
                {orgKind === "ngo" ? "Reach out" : "Unlock contact (1 credit)"}
              </button>
...
function EmpPositions({ orgKind }: { orgKind: "employer" | "ngo" }) {
  const positions = [
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 28, flexWrap: "wrap", gap: 16 }}>
        <div>
          ...
          <p style={{ fontSize: 16, color: "var(--ink-soft)", maxWidth: 540, margin: 0 }}>
            One row per opening. We tell you when something changes — we don't ask you to refresh.
          </p>
        </div>
      ...
      <div style={{ background: "var(--paper-card)", border: "1px solid var(--line-soft)", borderRadius: 22, overflow: "hidden" }}>
        {positions.map((p, i) => (
          <div key={i} style={{
            padding: "20px 22px",
            display: "grid", gridTemplateColumns: "2.4fr 1fr 1fr 1fr auto", gap: 18, alignItems: "center",
...
function SmallStat({ label, v, delta }: { label: string; v: string; delta: string }) {
  return (
    <div className="card">
