...
import { COUNTRIES, SOURCES } from "../data";
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <span className="display" style={{ fontSize: small ? 18 : 22, fontWeight: 600, letterSpacing: "-0.02em", color: "var(--ink)" }}>
        unmapped<span style={{ color: "var(--coral)" }}>.</span>
      </span>
    </div>
  );
}

export function SourceTag({ sourceId, label, mini = false }: { sourceId: string; label?: string; mini?: boolean }) {
  const [open, setOpen] = useState(false);
  const src = SOURCES[sourceId];
  return (
    <>
      <button className="source-btn" onClick={() => setOpen(true)}
        style={mini ? { fontSize: 10, padding: "3px 8px" } : undefined}>
        ...
        <div className="modal-backdrop" onClick={() => setOpen(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 10 }}>
              <span className="pill" style={{ background: "var(--green-pale)", color: "var(--green)", borderColor: "#c8debc" }}>{src.id}</span>
            ...
            <div style={{ borderTop: "1px solid var(--line-soft)", paddingTop: 16, fontSize: 13, color: "var(--ink-mute)", lineHeight: 1.6 }}>
              ...
              <p style={{ margin: 0 }}>
                Ingested through Unmapped's adapter pipeline (<code className="mono" style={{ fontSize: 12 }}>adapters/{sourceId.split('-')[0].toLowerCase()}.py</code>),
...
export function TopBar({ portal, onHome, country, onCountryChange, notifications }: {
  portal?: string; onHome: () => void;
  country?: string; onCountryChange?: (code: string) => void;
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        {country && onCountryChange && (
          <div style={{ position: "relative" }}>
            <button onClick={() => setCountryOpen(o => !o)} className="pill"
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => setNotifOpen(o => !o)} style={{
              ...
              {notifications.length > 0 && (
                <span style={{ position: "absolute", top: 6, right: 6, width: 8, height: 8, borderRadius: 999, background: "var(--coral)" }} />
              )}
            </button>
            {notifOpen && (
              <div className="fadein" style={{
                ...
                <div style={{ padding: "8px 10px", display: "flex", justifyContent: "space-between" }}>
                  ...
                  <button onClick={() => setNotifOpen(false)} style={{ border: "none", background: "transparent", fontSize: 11, color: "var(--ink-faint)", cursor: "pointer" }}>Mark all read</button>
                </div>
                {notifications.map((n, i) => (
                  <div key={i} style={{ padding: 12, borderRadius: 12, display: "flex", gap: 10, background: i === 0 ? "var(--paper-2)" : "transparent" }}>
                    <span style={{
...
export function FieldGroup({ label, children, style }: { label: string; children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <div style={style}>
