export interface Country {
  ...
  lmic: number; hci: number; youthUnemp: number; informal: number;
  internet: number; automation: number;
}

export interface Sector {
  sector: string; risk: number; growth: number; share: number; durable: string[];
}

export interface TalentProfile {
  id: string; alias: string; country: string; age: number;
  edu: string; availability: string; skills: string[];
  durable: string[]; vulnerable: string[]; match: number; sector: string;
}
...
export interface Notification {
  kind: string; icon: string; text: string; time: string;
}
...
const COUNTRY_SECTORS: Record<string, Sector[]> = {
  GHA: [
...
export function sectorsFor(code: string): Sector[] {
...
export const SOURCES: Record<string, Source> = {
  ...
  hci:        { id: "WB-HCI-2020",       title: "World Bank Human Capital Index",         blurb: "HCI 2020. Productivity of next-generation worker vs. full health & education benchmark." },
